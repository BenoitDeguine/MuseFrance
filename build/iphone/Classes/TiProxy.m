...
//Rare exceptions to indicate a bug in the _muse_france code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_muse_france = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_muse_france]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.muse_france.module"
